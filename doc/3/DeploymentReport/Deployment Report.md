...
که با PHP-FPM و Nginx ساخته شده. مزیت استفاده از این imageهای آماده این است که در صورت تعریف ورژن جدید توسط توسعه‌دهنده می‌توانیم به سرعت آن را به روز کنیم و احتمال خطا را پایین می‌آورد، همچنین این نسخه بخصوص به دلیل پیاده‌سازی با nginx قابلیت‌های بسیاری را به ما عرضه می‌کند، همگام با حجم و مصرف کم به دلیل استفاده از alpine-linux به عنوان base image این image.
